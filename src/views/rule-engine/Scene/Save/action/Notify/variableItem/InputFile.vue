<template>
    <j-input
        allowClear
        :placeholder="t('Notify.variableItem.InputFile.5rg4ubxxbsc0')"
        v-model:value="url"
        @change="onChange"
    >
        <template #addonAfter>
            ...
            >
                <j-button type="link" style="height: 30px">
                    ...
                    {{t('Notify.variableItem.InputFile.5rg4ubxxd1w0')}}
                </j-button>
            </j-upload>
        </template>
    </j-input>
...
const handleBeforeUpload = (file: any) => {
    ...
    if (!isJpgOrPng) {
        onlyMessage(t('Notify.variableItem.InputFile.5rg4ubxxda00'), 'error');
    }
    const isSize = file.size / 1024 / 1024 < 4;
    if (!isSize) {
        onlyMessage(t('Notify.variableItem.InputFile.limit'), 'error');
    }
