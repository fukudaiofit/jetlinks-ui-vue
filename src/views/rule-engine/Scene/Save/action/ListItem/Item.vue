<template>
    <div class="actions-item-warp">
        <j-form-item :name='["branches", branchesName, "then", thenName, "actions", name]' :rules='rules'>
            <div class="actions-item">
                <CheckItem v-bind='props' ref='checkItemRef'>
                    <div class="item-options-warp">
                        <div class="item-options-type" @click="onAdd">
                            <img style="width: 18px" :src="iconMap.get(
                                data?.executor === 'alarm'
                                    ? data?.alarm?.mode
                                    : data?.executor,
                            )
                        ...
                        <div class="item-options-content" v-if="data?.executor === 'alarm'">
                            <template v-if="data?.alarm?.mode === 'trigger'">
                                {{ t('action.ListItem.Item.5rg4saoj4q80') }}<j-button style="padding: 0" type="link"
                                    @click.stop="triggerVisible = true">{{ t('action.ListItem.Item.5rg4saoj60w0') }}</j-button>
                            </template>
                            <template v-else>
                                {{ t('action.ListItem.Item.5rg4saoj69c0') }}<j-button style="padding: 0" type="link"
                                    @click.stop="triggerVisible = true">{{ t('action.ListItem.Item.5rg4saoj60w0') }}</j-button>
                            </template>
                        </div>
                        <div class="item-options-content" v-else-if="data?.executor === 'notify'" @click="onType('notify')">
                            <template v-if="data?.notify?.notifyType === 'dingTalk'">
                                <template v-if="options?.provider === 'dingTalkRobotWebHook'">
                                    <div>
                                        {{ t('action.ListItem.Item.5rg4saoj6fo0') }}<span
                                            class="notify-text-highlight">{{ t('action.ListItem.Item.5rg4saoj6no0') }}</span>
                                ...
                                <template v-else>
                                    <div>
                                        ...
                                        <span class="notify-text-highlight">
                                            <img style="width: 18px" :src="itemNotifyIconMap.get(
                        ...
                        <div class="item-options-content" v-else-if="data?.executor === 'delay'" @click="onType('delay')">
                            {{ options?.name }}
                        </div>
                        <div class="item-options-content" v-else-if="data?.executor === 'device'" @click="onType('device')">
                            <template v-if="['fixed', 'context'].includes(data?.device?.selector)">
                                <div style='display: flex; align-items: center;'>
                                    ...
                                    <span
                                        v-if='!isBoolean(data?.options?.propertiesValue) && data?.options?.propertiesValue'>{{ t('action.ListItem.Item.5rg4saoj7tg0') }}
                                    </span>
