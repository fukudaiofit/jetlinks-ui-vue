<template>
  ...
  >
    ...
    >
      <j-form-item
        name='name'
        :label="t('Scene.Save.save.5rg58okopew0')"
        :rules="[
          { required: true, message: t('Scene.Save.save.5rg58okor1o0') },
          { max: 64, message: t('Scene.Save.save.5rg58okordc0') }
        ]"
      >
        <j-input v-model:value='formModel.name' :placeholder="t('Scene.Save.save.5rg58okor1o0')" />
      </j-form-item>
      <j-form-item
        :name='["trigger", "type"]'
        :label="t('Scene.Save.save.5rg58okork00')"
        :rules="[{ required: true, message: t('Scene.Save.save.5rg58okors00') }]"
      >
...
const title = computed(() => {
  return props.data?.id ? t('Scene.Save.save.5rg58okos4o0') : t('Scene.Save.save.5rg58okosac0')
})
