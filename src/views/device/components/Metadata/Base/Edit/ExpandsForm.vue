<template>
  <j-form-item :label="t('Base.Edit.ExpandsForm.5rnrcmns8vg0')" :name="name.concat(['source'])" v-if="type === 'product'" :rules="[
    { required: true, message: t('Base.Edit.ExpandsForm.5rnrcmnsaao0') },
  ]">
  ...
  <j-form-item :label="t('Base.Edit.ExpandsForm.5rnrcmnsalw0')" :name="name.concat(['type'])" :rules="[
    { required: true, message: t('Base.Edit.ExpandsForm.5rnrcmnsats0') },
  ]">
  ...
  <j-form-item :label="t('Base.Edit.ExpandsForm.5rnrcmnsb5k0')" v-if="config.length > 0">
  ...
  <j-form-item
    ...
    :name="name.concat(['metrics'])" :rules="[
      { validator: () => validateMetrics(_value.metrics), message: t('Base.Edit.ExpandsForm.5rnrcmnsbf40') }
    ]">
    <template #label>
      <j-space>
        {{t('Base.Edit.ExpandsForm.5rnrcmnsbn00')}}
        <j-tooltip :title="t('Base.Edit.ExpandsForm.5rnrcmnsbu00')">
...
const validateMetrics = (value: Record<any, any>[]) => {
  ...
  if (!flag) {
    return Promise.reject(new Error(t('Base.Edit.ExpandsForm.5rnrcmnsbf40')));
  }
