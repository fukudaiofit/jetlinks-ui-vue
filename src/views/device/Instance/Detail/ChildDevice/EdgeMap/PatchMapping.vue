...
const handleClick = async () => {
    if (!rightList.value.length) {
        message.warning(t('ChildDevice.EdgeMap.PatchMapping.5rcyc70pw4c0'));
    } else {
        const params: any[] = [];
        ...
        if (_props.deviceId) {
            if (filterParms && filterParms.length !== 0) {
                ...
                if (res.status === 200) {
                    message.success(t('ChildDevice.EdgeMap.PatchMapping.5rcyc70pwd00'));
                    _emits('save');
                }
            } else {
                message.error(t('ChildDevice.EdgeMap.PatchMapping.5rcyc70px0w0'));
            }
        } else {
            if (filterParms && filterParms.length !== 0) {
