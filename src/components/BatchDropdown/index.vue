    ...
    <div>
        <j-space v-if="_item.key && visible">
            ...
            <j-button type="link" @click="reload"
                ><AIcon type="RedoOutlined" />{{t('components.BatchDropdown.index.5rlc6t9b44k0')}}</j-button
            >
        </j-space>
        <j-dropdown :overlayStyle="{ zIndex: 1000 }" v-else>
            <j-button>{{t('components.BatchDropdown.index.5rlc6t9b58k0')}} <AIcon type="DownOutlined" /></j-button>
            <template #overlay>
